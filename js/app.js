...
// ── Popup No Fumar ──
let _nfViewDate = null;
let _nfDias = null; // Set de 'YYYY-MM-DD' donde se completó

async function _loadNoFumarDias() {
  if (_nfDias) return; // ya cargado
  _nfDias = new Set();
  try {
    const { loadAllCompletions } = await import('./habits.js');
    // Usar statsCompletions si está disponible, sino cargar
    if (!state.statsLoaded) await loadAllCompletions();
    const comps = state.statsLoaded ? state.statsCompletions : state.completions;
    Object.entries(comps).forEach(([dateStr, d]) => {
      if (!/^\d{4}-\d{2}-\d{2}$/.test(dateStr) || !d || Array.isArray(d)) return;
      const completados = d.completados || [];
      // Buscar hábito "no fumar" (sin distinción de mayúsculas)
      const noFumarHabit = state.allHabits.find(h =>
        h.name && h.name.toLowerCase().trim() === 'no fumar'
      );
      if (noFumarHabit && completados.includes(noFumarHabit.id)) {
        _nfDias.add(dateStr);
      }
    });
  } catch(e) { console.error('loadNoFumarDias:', e); }
}

function _renderNFCal() {
  const d = _nfViewDate;
  ...
  document.getElementById('nofumar-month-lbl').textContent = `${meses[month]} ${year}`;

  // Días semana
  const wdEl = document.getElementById('nofumar-weekdays');
  wdEl.innerHTML = ['L','M','X','J','V','S','D'].map(d =>
    `<div style="text-align:center;font-size:9px;color:var(--muted);letter-spacing:1px;padding:4px 0">${d}</div>`
  ).join('');

  // Primer día del mes (ajustar para que lunes=0)
  const firstDay = new Date(year, month, 1);
  let startOffset = firstDay.getDay() - 1;
  if (startOffset < 0) startOffset = 6;
  const daysInMonth = new Date(year, month + 1, 0).getDate();

  const calEl = document.getElementById('nofumar-cal');
  let html = '';
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    const isDone = _nfDias && _nfDias.has(ds);
    const isToday = ds === todayStr;
    let bg = 'transparent';
    let color = 'var(--muted)';
    let border = '1px solid transparent';
    if (isDone) { bg = 'rgba(143,179,57,0.2)'; color = 'var(--accent)'; border = '1px solid rgba(143,179,57,0.4)'; }
    if (isToday) { border = '1.5px solid var(--accent)'; }
    if (isFuture) { color = 'var(--border)'; }
    html += `<div style="text-align:center;padding:6px 2px;border-radius:8px;background:${bg};border:${border};font-size:12px;font-weight:${isDone||isToday?'700':'400'};color:${color}">${day}</div>`;
  }
  calEl.innerHTML = html;
}

window.openNoFumarPopup = async () => {
  lockScroll();
  _nfViewDate = new Date(today() + 'T12:00:00');
  document.getElementById('nofumar-overlay').style.display = 'block';
  document.getElementById('nofumar-count').textContent = '...';
  document.getElementById('nofumar-desde').textContent = 'Cargando...';
  await _loadNoFumarDias();
  // Contar total
  const total = _nfDias ? _nfDias.size : 0;
  document.getElementById('nofumar-count').textContent = total;
  // Primer día sin fumar
  if (_nfDias && _nfDias.size > 0) {
    const primero = Array.from(_nfDias).sort()[0];
    const d = new Date(primero + 'T12:00:00');
    document.getElementById('nofumar-desde').textContent = 'Desde el ' + d.toLocaleDateString('es-ES', {day:'numeric',month:'long',year:'numeric'});
  } else {
    document.getElementById('nofumar-desde').textContent = 'Aún no hay registros';
  }
  _renderNFCal();
};

window.closeNoFumarPopup = () => {
  document.getElementById('nofumar-overlay').style.display = 'none';
  unlockScroll();
};

window.nofumarPrevMonth = () => {
  _nfViewDate.setMonth(_nfViewDate.getMonth() - 1);
  _renderNFCal();
};

window.nofumarNextMonth = () => {
  _nfViewDate.setMonth(_nfViewDate.getMonth() + 1);
  _renderNFCal();
};
